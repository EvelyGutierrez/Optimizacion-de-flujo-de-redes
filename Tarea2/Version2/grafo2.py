...
def cabecera(aristas, eps=False):
    ...
    print("set xrange [-5.0:5.0]", file = aristas)
    print("set yrange [-5.0:5.0]", file = aristas)
...
class Grafo:
    ...
    def creaNodos(self, orden): # creando los nodos
        ...
        for nodo in range(self.n):
            self.x[nodo] = random()*5
            self.y[nodo] = random()*5 
    ...
    def grafica(self, plot): # imprimiendo el grafo con aristas
        ...
        with open(plot, "w") as aristas:
            ...
            for (v, w, p, c) in self.E:
                ...
                y1 = self.y[v]
                y2 = self.y[w]
                #self.E[(v, w)] = self.E[(w, v)] = 1
                flecha = "set arrow {:d} from {:f}, {:f} to {:f}, {:f} lw {:f} lt 5 lc rgb '{:s}' head".format(num,x1,y1,x2,y2,p,c)
